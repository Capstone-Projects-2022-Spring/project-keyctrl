...
io.on('connection', (socket) => {
  ...
  */
  ...
  socket.on('findRankedMatch', function () {
    if (!findRankedMatchPlayers.includes(socket.id)) {
      console.log(socket.id + " looking for match")
      findRankedMatchPlayers.push(socket.id)
    }

    //If there are more than 4 people queuing, begin a game
    if (findRankedMatchPlayers.length >= gameStartPlayers) {
      console.log('---- Match Found ----')
      console.log('---------------------')
      console.log('| Players: ')
      findRankedMatchPlayers.forEach(player => console.log("| " + player))
      console.log('---------------------')
      ...
      //Remove the first four players from the queue and emit them the findMatchSuccess event
      for (var i = 0; i < gameStartPlayers; i++) {
        var player = findRankedMatchPlayers.shift()
        io.to(player).emit('findMatchSuccess', lobby)
        console.log(player + ' joining match')
      }
    }
  ...
  socket.on('gameEnd', function (inRanked, photo, player, WPM, accountId, socialId, loggedIn, room) {
    console.log('gameEnd')
    if (matchResultsArray[room] == null) {
      matchResultsArray[room] = new Array()
    }
    matchResultsArray[room].push({ inRanked, photo, player, WPM, accountId, socialId, loggedIn })
    if (matchResultsArray[room].length === numClients[room]) {
      io.in(room).emit('matchResults', matchResultsArray[room])

      // api post for match history
      matchResultsArray[room] = sortLeaderBoard(matchResultsArray[room])
      if (matchResultsArray[room][0].inRanked) {
        callUpdateMatchHistory(1, matchResultsArray[room])
      } else {
        callUpdateMatchHistory(0, matchResultsArray[room])
      }
      numClients[room] = 0
...
function callUpdateMatchHistory(gamemode, array) {

  var options = {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    url: 'https://9x38qblue2.execute-api.us-east-1.amazonaws.com/dev/insertmatch',
    body: JSON.stringify({
      "matchType": gamemode,
      "player1": array[0].accountId,
      "player2": array[1].accountId,
      "player3": array[2].accountId,
      "player4": array[3].accountId,
      "player1wpm": array[0].WPM,
      "player2wpm": array[1].WPM,
      "player3wpm": array[2].WPM,
      "player4wpm": array[3].WPM,
      "player1name": array[0].player,
      "player2name": array[1].player,
      "player3name": array[2].player,
      "player4name": array[3].player
    })
  };
